...
from .PageParser import JabPageParser, jab_pattern

logger = Logger(config.log_dir).get_logger(__name__)
_page_parser = JabPageParser()
...
class ActressId:
    ...
    def _parse(self) -> None:
        if _page_parser._get_page_type() == Page.SINGLE_VIDEO:
            ...
            return
        if _page_parser._get_page_type() == Page.ACTRESS_HOME:
            ...
            self.actress_info = [ActessInfo(_actress_id, _actress_name)]
            return
        if _page_parser._get_page_type() == Page.MODEL_SELECT:
            self.actress_info = []
    ...
    def html_text(self, value: str) -> None:
        _page_parser._html_text = value
        if not (_page_parser._get_page_type() == Page.SINGLE_VIDEO 
                or _page_parser._get_page_type() == Page.ACTRESS_HOME 
                or _page_parser._get_page_type() == Page.MODEL_SELECT):
            logger.error(f"无效的页面,无法解析出演员id.")
            raise ValueError("无效的页面,无法解析出演员id.")
